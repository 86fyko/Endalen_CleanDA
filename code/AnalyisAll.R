...
#   It used the data created by different files: data_wranglin.R              # 
...
browser()# browse just to make sure that the rows are still selected correctly
names(allendalen)
...
write.csv(dryCAS, "./data/secondaryData/subsetDryasCassiope.csv", row.names = F)
ggplot(dryCAS, aes(x=SUBSITE, y=plot_biomass, color=as.factor(YEAR)))+
 ...
 # coord_flip()+
  theme_bw()
ggsave("./plots/veg/Biomass_Treatment_subsites_years_old.png", width=12, height =8, unit="cm")

write.csv(dryCAS_LH, "./data/secondaryData/dryCAS_LH.csv")
ggplot(dryCAS_LH, aes(x=YEAR, y=plot_biomass, color=TREATMENT))+
  ...
  ylab("Total Life Hits")+
  xlab("Year")+
  scale_x_continuous(breaks = c(2003, 2009, 2015, 2022)) +  # Set custom x-axis tick marks
  # coord_flip()+
  theme_bw()+
  theme(panel.spacing = unit(1, "lines")) 
ggsave("./plots/veg/Biomass_Treatment_subsites_years.png", width=12, height =12, unit="cm")
...
#  coord_flip()+
  theme_bw()
ggsave("./plots/veg/Biomass_Treatment_subsites_years2.png", width=12, height =8, unit="cm")
