...
# Plot the heatmap
ggplot(difference_data, aes(x = "", y = species, fill = difference)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "blue", high = "red", name = "Difference (2022 - 2003)") +
  facet_grid(cols = vars(TREATMENT), rows = vars(SUBSITE), scales = "free", space = "free") +
  theme_minimal() +
  theme(axis.text.x = element_blank()) +
  labs(x = NULL, y = "Species", title = "Difference in Abundance between 2003 and 2022")

# Plot the heatmap with color-coded increases and decreases
ggplot(difference_data, aes(x = "", y = species, fill = factor(sign(difference), levels = c(-1, 0, 1), labels = c("Decrease", "No Change", "Increase")))) +
  geom_tile(color = "white") +
  scale_fill_manual(values = c("red", "grey", "green"), name = "Change") +
  theme_minimal() +
  facet_grid(cols = vars(TREATMENT), rows = vars(SUBSITE), scales = "free", space = "free") +
  theme(axis.text.x = element_blank()) +
  labs(x = NULL, y = "Species", title = "Change in Abundance between 2003 and 2022")

ggplot(difference_data[complete.cases(difference_data$difference), ], aes(x = "", y = species, fill = factor(sign(difference), levels = c(-1, 0, 1), labels = c("Decrease", "No Change", "Increase")))) +
  geom_tile(color = "white") +
  scale_fill_manual(values = c("red", "grey", "green"), name = "Change") +
  theme_minimal() +
  facet_grid(cols = vars(TREATMENT), rows = vars(SUBSITE), scales = "free", space = "free") +
