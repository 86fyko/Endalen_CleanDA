...
allveg <- read.csv("./data/secondaryData/subsetDryasCassiope.csv", header=T)
names(allveg)
...
# Rename columns
# Shorten the names
if(nchar(column_names[7])>6){
  shortened_names <- sapply(column_names[-c(1:6)], function(name) {
    parts <- strsplit(name, "\\.")
    if (length(parts[[1]]) > 1) {
...
names(allveg_reordered)[-c(1:6)] <- shortened_names

allveg_reordered2022 <- subset(allveg_reordered, YEAR=="2022")
subsite_list <- split(allveg_reordered2022[,-c(4,6)], allveg_reordered2022$SUBSITE)


for (j in 1:length(subsite_list)){
    subsite_list[[j]] <- subsite_list[[j]] %>% replace(is.na(.), 0) # replace all Nas with 0s
    
    env.dry <- subset(env, PLOT%in%unique(subsite_list[[j]]$PLOT))
...
#    levels(env.dry$SUBSITE) <- c(0:3)
    ...
    vegdatapca.df <- as.data.frame(subsite_list[[j]])
    str(vegdatapca.df)
    ...
    points(spec.rda, col="darkgrey", cex=1)
  #  points(spec.rda, dis="sp", col="blue")
    text(spec.rda, dis="sp", col="blue", cex =0.7)
    text(spec.rda, dis="bp", col="black")
