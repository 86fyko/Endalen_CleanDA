...
Soil_moisture_wide <- pivot_wider(Soil_moisture_long[,-c(4:7)], names_from=date, values_from=mean_moisture) #add prefix to the dates
...
##### Merge dataframes####
allenv<- merge(Soil_moisture_wide[, c(1:3, 12)] ,NDVI_wide[,c(1,7)],by.x="PLOT", by.y="Plot", all=T) %>%
  merge(., allsnow[,c(3,5)], all=T)
...
snow_NDVI_BM<- merge(snow_NDVI, dryCAS_LH[,c(1,33)], by.y="PLOT", by.x="Plot",all=T)
...
lm_snowBM<- lm(plot_biomass ~ SNOW_DEPTH_CM ,data=subset(snow_NDVI, TREATMENT=="CTL" ) )
summary(lm_snowBM)
AIC(lm_snowND,lm_snowBM)

write.csv(allenv, "./allenvironmental.csv", row.names=F)
getwd()
#maybe I should use the overall mean moisture and NDVI since its not an analysis of NDVI or moisture
