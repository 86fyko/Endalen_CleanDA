...
snowdepth.plot <- ggplot(subset(allsnow, !(SUBSITE %in% c("BIS-H", "BIS-L"))), aes(x=SUBSITE, y=SNOW_DEPTH_CM, color=TREATMENT))+
  geom_boxplot()+
...
subset_sn <- subset(allsnow, !(SUBSITE %in% c("BIS-H", "BIS-L")))%>%subset(. , TREATMENT %in% c("CTL", "OTC"))
snowdepth.plot <- ggplot(subset_sn, aes(x = SUBSITE, y = SNOW_DEPTH_CM, color = TREATMENT)) +
  geom_boxplot() +
  theme_bw()+
  geom_point(aes(x = SUBSITE, y = SNOW_MAST), color = "black") +
  scale_colour_viridis_d(
...
anova_result <- aov(SNOW_DEPTH_CM ~ SUBSITE + TREATMENT, data = subset_sn)
...
lm_model <- lm(SNOW_DEPTH_CM ~ SUBSITE + TREATMENT, data = subset_sn)
summary(lm_model)
...
average_snow_depth <- aggregate(SNOW_DEPTH_CM ~ SUBSITE + TREATMENT, data = subset_sn, 
...
average_snow_depth$SE <- average_snow_depth$sd / sqrt(nrow(subset_sn) / nrow(average_snow_depth))
...
# merge the data
snow_data_cor <- merge(result_data, subset_sn[,c(1:4, 9)], all.y = T, by.x=c( "TREATMENT", "SUBSITE","Site"), by.y=c( "TREATMENT", "SUBSITE","PLOT"))
